...
def scrape_transcript(url):
    ...
    timestamped_transcripts = YouTubeTranscriptApi.get_transcript(video_id)

    text = []
    start_time = []
    duration = []
    for transcript in timestamped_transcripts:
        text.append(transcript['text'])
        start_time.append(transcript['start'])
        duration.append(transcript['duration'])

    dict = {'transcribed_text': text, 'start_time': start_time, 'duration': duration} 
    transcript_df = pd.DataFrame(dict)
